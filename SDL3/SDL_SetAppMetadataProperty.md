...
- [`SDL_PROP_APP_METADATA_NAME_STRING`](SDL_PROP_APP_METADATA_NAME_STRING):
  ...
  applets, etc. If not provided by the app, SDL will attempt to pick a
  reasonable default (the app's binary's name if the platform can provide
  it, or "SDL Application" if all else fails).
