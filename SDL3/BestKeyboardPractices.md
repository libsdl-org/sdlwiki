...
In this case, it's useful to assume that text is generated per-keypress and what we want to know is, within possibly severe limitations, what character should be generated by pressing a physical key plus modifiers like Shift, Caps Lock, etc.

Obviously there are many keys that _don't_ generate a character, or characters that are generated by pressing multiple keys (or navigating through IME interfaces that don't map to specific keypresses at all), so this is niche functionality and not how one should accept input in a general sense.
