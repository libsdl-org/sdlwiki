...
{|
|'''sock'''
|the UDP socket to bind an address to a channel on.
|-
|'''channel'''
|the channel of the socket to bind to, or -1 to use the first available channel.
|-
|'''address'''
|the address to bind to the socket's channel.
|}
...
If <code>channel</code> is -1, then the first unbound channel that has not
yet been bound to the maximum number of addresses will be bound with the
given address as it's primary address.
