...
{|
|'''port'''
|the UDP port to bind this socket to.
|}
...
If <code>port</code> is non-zero, the UDP socket is bound to a local port.

The <code>port</code> should be given in native byte order, but is used
internally in network (big endian) byte order, in addresses, etc. This
allows other systems to send to this socket via a known port.
