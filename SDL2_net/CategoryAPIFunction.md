# CategoryAPIFunction

<!-- END CATEGORY DOCUMENTATION -->

<!-- DO NOT HAND-EDIT CATEGORY LISTS, THEY ARE AUTOGENERATED AND WILL BE OVERWRITTEN, BASED ON TAGS IN INDIVIDUAL PAGE FOOTERS. EDIT THOSE INSTEAD. -->
<!-- BEGIN CATEGORY LIST -->
- [SDLNet_AddSocket](SDLNet_AddSocket)
- [SDLNet_AllocPacket](SDLNet_AllocPacket)
- [SDLNet_AllocPacketV](SDLNet_AllocPacketV)
- [SDLNet_AllocSocketSet](SDLNet_AllocSocketSet)
- [SDLNet_CheckSockets](SDLNet_CheckSockets)
- [SDLNet_DelSocket](SDLNet_DelSocket)
- [SDLNet_FreePacket](SDLNet_FreePacket)
- [SDLNet_FreePacketV](SDLNet_FreePacketV)
- [SDLNet_FreeSocketSet](SDLNet_FreeSocketSet)
- [SDLNet_GetError](SDLNet_GetError)
- [SDLNet_GetLocalAddresses](SDLNet_GetLocalAddresses)
- [SDLNet_Init](SDLNet_Init)
- [SDLNet_Linked_Version](SDLNet_Linked_Version)
- [SDLNet_Quit](SDLNet_Quit)
- [SDLNet_ResizePacket](SDLNet_ResizePacket)
- [SDLNet_ResolveHost](SDLNet_ResolveHost)
- [SDLNet_ResolveIP](SDLNet_ResolveIP)
- [SDLNet_SetError](SDLNet_SetError)
- [SDLNet_TCP_Accept](SDLNet_TCP_Accept)
- [SDLNet_TCP_AddSocket](SDLNet_TCP_AddSocket)
- [SDLNet_TCP_Close](SDLNet_TCP_Close)
- [SDLNet_TCP_DelSocket](SDLNet_TCP_DelSocket)
- [SDLNet_TCP_GetPeerAddress](SDLNet_TCP_GetPeerAddress)
- [SDLNet_TCP_Open](SDLNet_TCP_Open)
- [SDLNet_TCP_OpenClient](SDLNet_TCP_OpenClient)
- [SDLNet_TCP_OpenServer](SDLNet_TCP_OpenServer)
- [SDLNet_TCP_Recv](SDLNet_TCP_Recv)
- [SDLNet_TCP_Send](SDLNet_TCP_Send)
- [SDLNet_UDP_AddSocket](SDLNet_UDP_AddSocket)
- [SDLNet_UDP_Bind](SDLNet_UDP_Bind)
- [SDLNet_UDP_Close](SDLNet_UDP_Close)
- [SDLNet_UDP_DelSocket](SDLNet_UDP_DelSocket)
- [SDLNet_UDP_GetPeerAddress](SDLNet_UDP_GetPeerAddress)
- [SDLNet_UDP_Open](SDLNet_UDP_Open)
- [SDLNet_UDP_Recv](SDLNet_UDP_Recv)
- [SDLNet_UDP_RecvV](SDLNet_UDP_RecvV)
- [SDLNet_UDP_Send](SDLNet_UDP_Send)
- [SDLNet_UDP_SendV](SDLNet_UDP_SendV)
- [SDLNet_UDP_SetPacketLoss](SDLNet_UDP_SetPacketLoss)
- [SDLNet_UDP_Unbind](SDLNet_UDP_Unbind)
<!-- END CATEGORY LIST -->

