...
Either you can compile and install it from source (via the usual ./configure && make && make install procedure). Or install it via some kind of packaging system which ships premade packages for SDL. For example, both Fink and [MacPorts](http://www.macports.org/) include packages which are suitable. You can learn more about them on their respective sites.
...
One good strategy can be found here:  [Properly bundling .frameworks in your application package](http://overooped.com/post/42240519/properly-bundling-frameworks-in-your-application)
